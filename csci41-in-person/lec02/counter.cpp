...
void Counter::reset(int newCount) {
  // if (newCount < 0) return; // we make count private so we can do this sort of thing
  count = newCount;
}

int Counter::getCount() const {
  // count = 42; // not allowed when we use const!
  return count;
}
...
int main() {
  ...
  // OR c.operator+(c2)
  Counter c3 = c + c2; // secretly turned into c.operator+(c2)
